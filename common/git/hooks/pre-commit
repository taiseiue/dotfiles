...
check_conflict() {
  # git config --local pre-commit.skip-conflict-checkがtrueならチェックをスキップする
  if git config --local pre-commit.skip-conflict-check | grep -q -E '^(1|true|yes)$'; then
    return 0
  fi
...
check_protected_branch() {
  # git config --local pre-commit.allow-mainpushがtrueならチェックをスキップする
  if git config --local pre-commit.allow-mainpush | grep -q -E '^(1|true|yes)$'; then
    return 0
  fi
